...
```
POST /validation/api/items/add 호출
POST /validation/api/items/add 성공
```
...
```
Resolved [org.springframework.http.converter.HttpMessageNotReadableException: JSON parse error: Cannot deserialize value of type `java.lang.Integer` from String "A": not a valid `java.lang.Integer` value]
```
...
`HttpMessageConverter`에서 요청 `JSON`을 `ItemSaveForm` 객체로 생성하는데 실패한다.
...
Field error in object 'itemSaveDto' on field 'quantity': rejected value [1000000]; 
codes [Max.itemSaveDto.quantity,Max.quantity,Max.java.lang.Integer,Max]; 
arguments [org.springframework.context.support.DefaultMessageSourceResolvable: 
codes [itemSaveDto.quantity,quantity]; arguments []; default message [quantity],9999]; 
default message [9999 이하여야 합니다]
```
...
스프링이 이 객체를 `JSON`으로 변환해서 클라이언트에 전달했다.
여기서는 예시로 보여주기 위해서 검증 오류 객체들을 그대로 반환했다.
실제 개발할 때는 이 객체들을 그대로 사용하지 말고, **필요한 데이터만 뽑아서 별도의 API 스펙을 정의하고 그에 맞는 객체를 만들어서 반환**해야 한다.
...
따라서 `HttpMessageConverter`의 작동이 성공해서 `ItemSaveForm`객체를 만들어야 `@Valid`, `@Validated`가 적용된다.
